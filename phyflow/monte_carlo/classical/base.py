...
class MonteCarloSampler(nn.Module, ABC):
    ...
    def high_precision_derivative(
            ...
            spacing: float = 1.0,
            denoise: bool = False,
            denoise_window_length: int = 15,
            denoise_polyorder: int = 4
    ) -> Tensor:
        ...
        forward/backward differences at the boundaries. Optionally applies
        Savitzky-Golay filtering for denoising prior to differentiation.
        ...
        Args:
            ...
            denoise (bool, optional): If True, applies Savitzky-Golay filtering
                                      to `seq` before computing the derivative.
                                      Requires `scipy` to be installed.
                                      Defaults to False.
            denoise_window_length (int, optional): The length of the filter window
                                                   for Savitzky-Golay smoothing. Must
                                                   be a positive odd integer and
                                                   typically >= `denoise_polyorder` + 1.
                                                   Used only if `denoise` is True.
                                                   Defaults to 15.
            denoise_polyorder (int, optional): The order of the polynomial used
                                               to fit the samples in the Savitzky-Golay
                                               filter. Must be non-negative and less
                                               than `denoise_window_length`. Used only
                                               if `denoise` is True. Defaults to 4.
        ...
        Raises:
            ValueError: If `seq` is not a 1D tensor, if its length `N` is less than 5,
                        or if denoising parameters are invalid.
            ImportError: If `denoise` is True and `scipy` is not installed.
        """
        # --- Input Validation ---
        if not isinstance(seq, torch.Tensor):
            raise TypeError(f"Input 'seq' must be a torch.Tensor, got {type(seq)}")
        if seq.ndim != 1:
        ...
        if denoise:
            # Check if SciPy is available for calculating coefficients
            if not _SCIPY_AVAILABLE:
                raise ImportError(
                    "The 'scipy' package is required for denoising. "
                    "Please install it (`pip install scipy`) and try again."
                )

            # Validate denoising parameters
            if not isinstance(denoise_window_length, int) or denoise_window_length <= 0:
                raise ValueError("denoise_window_length must be a positive integer.")
            if denoise_window_length % 2 == 0:
                raise ValueError("denoise_window_length must be odd.")
            if not isinstance(denoise_polyorder, int) or denoise_polyorder < 0:
                raise ValueError("denoise_polyorder must be a non-negative integer.")
            if denoise_polyorder >= denoise_window_length:
                raise ValueError("denoise_polyorder must be less than denoise_window_length.")
            if denoise_window_length > N:
                raise ValueError(f"denoise_window_length ({denoise_window_length}) "
                                 f"cannot be larger than sequence length ({N}).")

            # Calculate Savitzky-Golay filter coefficients for smoothing (deriv=0)
            try:
                coeffs_np = savgol_coeffs(denoise_window_length, denoise_polyorder, deriv=0)
                coeffs = torch.from_numpy(coeffs_np).to(seq.dtype).to(seq.device)
            except Exception as e:
                raise ValueError(f"Failed to compute Savitzky-Golay coefficients "
                                 f"with window={denoise_window_length}, "
                                 f"polyorder={denoise_polyorder}. Error: {e}")

            # Apply the filter using 1D convolution
            kernel = coeffs.view(1, 1, denoise_window_length)
            seq_reshaped = seq.view(1, 1, N)

            # Calculate padding amount
            padding_val = (denoise_window_length - 1) // 2

            # Manually pad the input tensor using reflection padding
            # F.pad requires padding amounts for each dimension starting from the last.
            # For our (1, 1, N) tensor, we pad the last dimension (length)
            # with (padding_left, padding_right).
            padded_seq = F.pad(seq_reshaped, (padding_val, padding_val), mode='replicate')

            # Apply convolution with padding=0, as padding is now handled manually
            smoothed_seq_reshaped = F.conv1d(
                padded_seq,
                kernel,
                padding=0  # Use padding=0 since input is pre-padded
            )

            # Reshape back to 1D: (N,)
            # The output size of conv1d on the padded input should match the original N
            seq_to_diff = smoothed_seq_reshaped.squeeze()
            # print("Applied Savitzky-Golay denoising via manual padding.") # Uncomment for debugging

        else:
            # Use the original sequence if denoising is disabled
            seq_to_diff = seq

        # --- Derivative Calculation ---
        # Allocate output tensor based on the potentially smoothed sequence
        deriv = torch.empty_like(seq_to_diff)
        h = spacing

        # --- Interior points: 4th-order central difference ---
        # Formula: f'(x) ≈ [-f(x+2h) + 8f(x+h) - 8f(x-h) + f(x-2h)] / (12h)
        # Uses the (potentially smoothed) seq_to_diff tensor
        f_ip2 = seq_to_diff[4:]  # f[i+2] for i in [2, N-3]
        f_ip1 = seq_to_diff[3:-1]  # f[i+1] for i in [2, N-3]
        f_im1 = seq_to_diff[1:-3]  # f[i-1] for i in [2, N-3]
        f_im2 = seq_to_diff[0:-4]  # f[i-2] for i in [2, N-3]
        ...
        deriv[0] = (-3 * seq_to_diff[0] + 4 * seq_to_diff[1] - seq_to_diff[2]) / (2 * h)

        # Point i=1: 2nd-order central difference
        # Formula: f'(x₁) ≈ [f(x₂) - f(x₀)] / (2h)
        deriv[1] = (seq_to_diff[2] - seq_to_diff[0]) / (2 * h)

        # Point i=N-2: 2nd-order central difference
        # Formula: f'(x_{N-2}) ≈ [f(x_{N-1}) - f(x_{N-3})] / (2h)
        deriv[-2] = (seq_to_diff[-1] - seq_to_diff[-3]) / (2 * h)

        # Right boundary (i=N-1): 2nd-order backward difference
        # Formula: f'(x_{N-1}) ≈ [3f(x_{N-1}) - 4f(x_{N-2}) + f(x_{N-3})] / (2h)
        deriv[-1] = (3 * seq_to_diff[-1] - 4 * seq_to_diff[-2] + seq_to_diff[-3]) / (2 * h)
