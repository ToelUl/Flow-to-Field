...
class RotaryEmbedding2D(nn.Module):
    ...
    def __init__(self, dim: int, base: int = 10000):
        ...
        inv_freq = 1.0 / (self.base**freqs_for_half)
...
class MSAWithRoPE(nn.Module):
    ...
    def __init__(self,
        ...
        self.rope = RotaryEmbedding2D(dim=self.head_dims)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # 4. Reshape output and add residual connection.
        # Shape: (B, num_heads, N, head_dims) -> (B, N, C) -> (B, C, H, W)
...
class MSA(nn.Module):
    """A Multi-Head Self-Attention module with a residual connection.

    Uses a Depthwise Separable Convolution for QKV projection. It also includes
    a fallback for older PyTorch versions without `scaled_dot_product_attention`.

    Attributes:
        is_identity: If True, the module acts as an identity function.
        num_heads: The number of attention heads.
        head_dims: The dimension of each attention head.
        scale: The scaling factor for the dot product.
        qkv_projection: The layer to project input to Q, K, V.
    """

    def __init__(self, channels: int, num_heads: int = 8, qkv_bias: bool = True, padding_mode: str = "circular"):
        """Initializes the Attention module.

        Args:
            channels: The number of input and output channels.
            num_heads: The number of attention heads. Must divide `channels`.
            qkv_bias: If True, adds a learnable bias to the QKV projection.
            padding_mode: The padding mode for the QKV projection convolution.
        """
        super().__init__()
        if channels <= 0:
            self.is_identity = True
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        B, C, H, W = x.shape
        qkv = self.qkv_projection(x).chunk(3, dim=1)
        q, k, v = map(lambda t: t.view(B, self.num_heads, self.head_dims, H * W).transpose(-1, -2), qkv)

        # Use efficient attention if available, otherwise fallback.
        try:
            out = F.scaled_dot_product_attention(q, k, v, is_causal=False)
        except (AttributeError, TypeError):
...
class DiResBlock(nn.Module):
    """A Diffusion Residual Block with adaptive layer normalization.
    ...
    Attributes:
        ...
        attn_eca: Efficient Channel Attention for the attention output.
    ...
    def forward(self, x: Tensor, emb: Tensor) -> Tensor:
        """Forward pass for the DiResBlock.
...
class DiResUnet(nn.Module):
    """A U-Net model built with DiResBlocks for diffusion tasks.

    This model architecture follows the standard U-Net pattern with an encoder,
    a bottleneck, and a decoder with skip connections. It uses DiResBlocks as
    ...
    def __init__(
        ...
        num_heads: int = 8,
        num_groups: int = 4,
        time_emb_dims: Optional[int] = None,
        cond_emb_dims: Optional[Sequence[int]] = None,
        final_emb_dims: Optional[int] = None,
        padding_mode: str = "circular",
    ):
        """Initializes the DiResUnet model.
        ...
        Args:
            ...
            channel_mult: A sequence of multipliers for the channel count at each
                level of the U-Net.
            num_res_blocks: The number of DiResBlocks at each level.
            dropout: The dropout rate used in the DiResBlocks.
            num_heads: The number of attention heads in the DiResBlocks.
            num_groups: The number of groups for GroupNorm in the DiResBlocks.
            time_emb_dims: The dimension for the sinusoidal time embedding.
                Defaults to `model_channels * 4`.
            cond_emb_dims: A sequence of dimensions for conditional embeddings.
                If None, no conditioning is used.
            final_emb_dims: The final dimension of the combined time and
                conditional embeddings passed to the DiResBlocks. Defaults to
                `time_emb_dims`.
        ...
        # --- Embedding Setup ---
        time_emb_dims = time_emb_dims or model_channels * 4
        if time_emb_dims % 2 != 0:
            time_emb_dims += 1
        self.time_emb_dims = time_emb_dims
        final_emb_dims = final_emb_dims or self.time_emb_dims
        self.final_emb_dims = final_emb_dims

        self.time_embedder = SinusoidalPosEmb(self.time_emb_dims)
        self.time_mlp = nn.Sequential(
            nn.Linear(self.time_emb_dims, self.final_emb_dims),
        )

        self.use_condition = cond_emb_dims is not None and len(cond_emb_dims) > 0
        if self.use_condition:
            self.cond_emb_dims = [(d + (d % 2)) for d in cond_emb_dims]  # Ensure even dims
            self.cond_embedders = nn.ModuleList([SinusoidalPosEmb(d) for d in self.cond_emb_dims])
            self.cond_mlps = nn.ModuleList([
                nn.Sequential(
                    nn.Linear(d, self.final_emb_dims),
                ) for d in self.cond_emb_dims
            ])
            self.embedding_attention = ConditionalSAWithRoPE(dim=self.final_emb_dims)

        # --- U-Net Backbone Construction ---
        block_args = {
            "emb_dims": self.final_emb_dims,
            "num_heads": num_heads,
        ...
        for i in range(self.num_levels):
            self.down_blocks.append(
                nn.ModuleList([DiResBlock(channels=current_ch, **block_args) for _ in range(num_res_blocks)]))
        ...
        self.middle_blocks = nn.ModuleList([
            DiResBlock(channels=ch, **block_args),
            DiResBlock(channels=ch, **block_args)
        ])
        ...
        for i in reversed(range(self.num_levels)):
            ...
            self.up_blocks.append(
                nn.ModuleList([DiResBlock(channels=target_ch, **block_args) for _ in range(num_res_blocks)]))
    ...
    def forward(self, x: Tensor, time: Tensor, conditions: Optional[Sequence[Tensor]] = None) -> Tensor:
        """The forward pass of the DiResUnet model.
        ...
        t_emb = self.time_mlp(self.time_embedder(time))
        final_emb = t_emb

        if self.use_condition:
            if conditions is None or len(conditions) != len(self.cond_emb_dims):
                raise ValueError(f"Expected {len(self.cond_emb_dims)} conditions, but got {len(conditions) if conditions else 0}")
            ...
            for i, cond_tensor in enumerate(conditions):
                ...
                c_emb = self.cond_mlps[i](self.cond_embedders[i](cond_tensor.to(x.device)))
        ...
        for i in range(self.num_levels):
            ...
            # Perform feature refinement using the DiResBlocks for this level.
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Test DiResBlock
        print("\nTesting DiResBlock...")
        di_res_block1 = DiResBlock(channels=helper_C_in, emb_dims=helper_emb_dim, num_heads=4, num_groups=4).to(device)
        ...
        di_res_block2 = DiResBlock(channels=helper_C_in, emb_dims=helper_emb_dim, num_heads=8, num_groups=8).to(device)
        ...
        # assert output2.shape == (helper_test_B, helper_C_out, helper_H, helper_W), f"DiResBlock (C_in!=C_out) shape mismatch: {output2.shape}"
        print("✅ DiResBlock (C_in!=C_out) PASSED.")
    ...
    try:
        unet_model = DiResUnet(
            ...
            time_emb_dims=TIME_EMB_DIM,
            final_emb_dims=FINAL_EMB_DIM,
            num_heads=NUM_HEADS,
            num_groups=NUM_GROUPS,
            dropout=DROPOUT
        ).to(device)
    ...
    try:
        unet_no_cond = DiResUnet(
            ...
            cond_emb_dims=None,
            time_emb_dims=TIME_EMB_DIM, final_emb_dims=FINAL_EMB_DIM,
